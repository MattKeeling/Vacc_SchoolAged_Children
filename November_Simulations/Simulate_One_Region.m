...
% In this version, negative values of ComplianceT are required R values !!

GETTING_OLDER=1;

ODEs=@LeakyVacc_ODEs; % this function could be compiled to mex code to make everything run faster.
...
tmp=[1 0.926 0.921 0.944 0.943 0.947 0.927 0.877 0.914 0.929 0.968];
...
if isempty(FS)
    ...
    [T2,S,E,D,nD2,U , Ratio, Da, FinalState]=ODEs(MaxType, UK_from_toH*0 , gamma*(UK_from_toH + UK_from_toW + UK_from_toS + UK_from_toO), INC_P, Early_gamma, Susceptibility, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 2, -1);
    ...
    [T,S,E,D,nD,U , Ratio, Da, FinalState]=ODEs(MaxType, gamma*UK_from_toH*Z , gamma*(UK_from_toW + UK_from_toS + UK_from_toO), INC_P, Early_gamma, Susceptibility2, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 71-Start_Date, [FinalState]);
    T=T+Start_Date;
    
    %Self Isolation for 4 days
    [t,s,e,d,nd,u , ratio, Da, FinalState]=ODEs(MaxType, gamma*UK_from_toH*Z, gamma*(UK_from_toW + UK_from_toS + UK_from_toO), INC_P, gamma, Susceptibility3, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 4, [FinalState]);
    ...
    [t,s,e,d,nd,u , ratio, Da, FinalState]=ODEs(MaxType, gamma*new_UK_from_toH*Z, gamma*(new_UK_from_toW + new_UK_from_toS + new_UK_from_toO), INC_P, gamma, Susceptibility3, Detection, TAU, nV_Beta, nV_Speed, 0, Region_PP(Region,:)' , V1(1,:), V2(1,:), V3(1,:), Transmission_Reduction, VEffI, VEffS, RatioPf, WaningSpeed, WaningEfficacy, 4, [FinalState]);
    T=[T; T(end)+t(2:end)]; S=[S; s(2:end,:)]; E=[E; e(2:end,:)]; D=[D; d(2:end,:)]; nD=[nD; nd(2:end,:)]; U=[U; u(2:end,:)]; Ratio=[Ratio; ratio(2:end,:,:)];
...
for W=W_start:length(Run_stop)
    ...
    if GETTING_OLDER
        if Told<=datenum(y,9,4) & T(end)>datenum(y,9,4) % Move up year !!
            ...
            FS=FinalState;
            for A=1:21
                PopSize=sum(FS(A:L:end))-sum(FS(SPV+A+[0 1 9]*L));  %substract numV1, numV2 and Boosters
    ...
    if Told<=datenum(y,8,4) & T(end)>datenum(y,8,4)  % Reset booster status in August
    ...
    %fprintf(1,'%d %d %d',round(mV1(15)),round(mV2(15)),round(mV3(15)));
    ...
    [t,s,e,d,nd,u , ratio, Da, FinalState]=ODEs(MaxType, Extra_Beta*gamma*new_UK_from_toH*Z, Extra_Beta*gamma*(new_UK_from_toW + new_UK_from_toS + new_UK_from_toO), INC_P, gamma, ...
...
TYPE=2;
    LL=(TYPE-1)*L;
    [~, nHospital(:,aa+LL), inHospital(:,aa+LL), nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), INF(:,aa+LL)]=ODE_to_Obs(T, nDCH(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P,...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, Sympt_2_hosp,  WALES_FLAG);
    [~, ~, ~, nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), ~]=ODE_to_Obs(T, nDCD(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P,...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, Sympt_2_hosp,  WALES_FLAG);
...
for TYPE=3:MaxType
    ...
    [~, nHospital(:,aa+LL), inHospital(:,aa+LL), nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), INF(:,aa+LL)]=ODE_to_Obs(T, nDCH(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P,...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, Sympt_2_hosp_Delta,  WALES_FLAG);
    [~, ~, ~, nICU(:,aa+LL), inICU(:,aa+LL), nDeaths(:,aa+LL), ~]=ODE_to_Obs(T, nDCD(:,aa+LL), nE(:,aa+LL), Region, h_factor(TYPE),i_factor(TYPE),d_factor(TYPE),h_stretch(TYPE),i_stretch(TYPE),INC_P,...
        Assumed_Delay_Reporting_Deaths, Distribution_Hosp_Time', Distribution_HospICU_Time', Distribution_ICU_Time', Distribution_Symptoms_to_Hospital, Distribution_Symptoms_to_ICU, Distribution_Hopital_to_Death,...
        Hosp_2_Death, Sympt_2_critcal, Sympt_2_hosp_Delta,  WALES_FLAG);
end
