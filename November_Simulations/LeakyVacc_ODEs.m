...
if InitialState(1)<0 
    ...
    % extra S classes and 3 immune class (V1,V2 & R); and another 6 for
    % Waning & fully protected, and +1 for if you've had a booster
    InitialState=[InitialState zeros(1,(MaxType-1)*(length(InitialState)-L)) zeros(1,L*10)];
...
mVEffI=mVEffI/tmp;  mVEffS=mVEffS/tmp; mWaneEff=mWaneEff/tmp;

for A=1:L
    Ratios(:,A,1)=pop(:,A);
    Ratios(:,A,2)=(1-mVEffI(A,1)).*pop(:,SPV+2*L+A);
    Ratios(:,A,3)=(1-mVEffI(A,2)).*(pop(:,SPV+3*L+A)+pop(:,SPV+5*L+A));
    Ratios(:,A,4)=(1-mVEffI(A,3)).*pop(:,SPV+4*L+A);
    Ratios(:,A,5)=(1-mWaneEff(2))*pop(:,SPV+6*L+A);
    ...
    RatioS(:,A,3)=(1-mVEffS(A,2)).*(pop(:,SPV+3*L+A)+pop(:,SPV+5*L+A));
...
for Vtype=MaxType:-1:1
    ...
    nD(:,a+LL)=(sum(RatioS,3)./sum(Ratios,3)).*(ones(size(pop,1),1)*d(a)').*(alpha*m*(pop(:,SP+L+a+3*(m-1)*L)+pop(:,SP+2*L+a+3*(m-1)*L)+pop(:,SP+3*L+a+3*(m-1)*L)+pop(:,SP+6*L+a+3*m*L+(m-1)*L)));
end
FinalState=pop(end,:);
...
l=5;               WaningSpeed=parameter(s+[1:l]);           s=s+l;    % 1 is from REC to 2nd REC, 2 is from VACC to 2nd Vacc; 3 & 4 are to waning class.
...
wV=pop(SPV+5*L+[1:L]);  WaneV=pop(SPV+6*L+[1:L]); 
wR=pop(SPV+7*L+[1:L]);  WaneR=pop(SPV+8*L+[1:L]); 
...
wVPos=SPV+5*L; WaneVPos=SPV+6*L; 
wRPos=SPV+7*L; WaneRPos=SPV+8*L; 
Boost=SPV+9*L;
...
SALL=S + (1-VEffI(:,Vtype,1)).*nV1 + (1-VEffI(:,Vtype,2)).*(nV2+wV) + (1-VEffI(:,Vtype,3)).*nV3 + (1-VEffI(:,Vtype,4)).*(wR+RRA-RR(:,Vtype)) + (1-WaningEfficacy(Vtype,2))*WaneV + (1-WaningEfficacy(Vtype,1))*WaneR;
...
Trans_Scaling=(1*S + Transmission_Reduction(:,Vtype,1).*(1-VEffI(:,Vtype,1)).*nV1 + ...
    Transmission_Reduction(:,Vtype,2).*(1-VEffI(:,Vtype,2)).*(nV2+wV) + Transmission_Reduction(:,Vtype,3).*(1-VEffI(:,Vtype,3)).*nV3 + ...
    Transmission_Reduction(:,Vtype,4).*(1-VEffI(:,Vtype,4)).*(wR+RRA-RR(:,Vtype)) + ...
    (1-WaningEfficacy(Vtype,1))*WaningEfficacy(Vtype,7)*WaneR + (1-WaningEfficacy(Vtype,2))*WaningEfficacy(Vtype,8)*WaneV)./SALL;

Symp_Scaling=(1*S + (1-VEffS(:,Vtype,1)).*nV1 + (1-VEffS(:,Vtype,2)).*(nV2+wV) + (1-VEffS(:,Vtype,3)).*nV3 + (1-VEffS(:,Vtype,4)).*(wR+RRA-RR(:,Vtype)) + ...
    (1-WaningEfficacy(Vtype,1))*WaneR + (1-WaningEfficacy(Vtype,2))*WaneV)./SALL;
...
dPop(a+wVPos)= - (1-VEffI(:,Vtype,2)).*(InfF + InfS1 + InfS2 + InfSQ).*wV(a)./N(a);       
dPop(a+WaneRPos)= - (1-WaningEfficacy(Vtype,1))*(InfF + InfS1 + InfS2 + InfSQ).*WaneR(a)./N(a);       
dPop(a+WaneVPos)= - (1-WaningEfficacy(Vtype,2))*(InfF + InfS1 + InfS2 + InfSQ).*WaneV(a)./N(a);
for Typ=1:MaxType
    if Typ~=Vtype
...
dPop(a+wVPos)= dPop(a+wVPos) - (1-VEffI(:,Vtype,2)).*(InfF + InfS1 + InfS2 + InfSQ).*wV(a)./N(a);       
dPop(a+WaneRPos)= dPop(a+WaneRPos) - (1-WaningEfficacy(Vtype,1))*(InfF + InfS1 + InfS2 + InfSQ).*WaneR(a)./N(a);       
dPop(a+WaneVPos)= dPop(a+WaneVPos)- (1-WaningEfficacy(Vtype,2))*(InfF + InfS1 + InfS2 + InfSQ).*WaneV(a)./N(a);       
for Typ=1:MaxType
    if Typ~=Vtype
        SP=(Typ-1)*(9*L+4*m*L)+L; ps=(SP+8*L+[1:L]+4*m*L);
...
% NOW DO VACCINATION AND OTHER REGULAR PROCESSES
...
WSR=WaningSpeed(1)*WaningSpeed(3)./(WaningSpeed(1)+WaningSpeed(3));
if WaningSpeed(1)*WaningSpeed(3)==0  WSR=0; end
...
dPop(a+wRPos)=dPop(a+wRPos) - pop(a+wRPos)*WaningSpeed(3);
dPop(a+WaneRPos)=dPop(a+WaneRPos) + pop(a+wRPos)*WaningSpeed(3);

%now 1 dose vaccine
dPop(a+nVPos1)=dPop(a+nVPos1) - pop(a+nVPos1)*WS;
dPop(a+WaneVPos)=dPop(a+WaneVPos) + pop(a+nVPos1)*WS;

%and 2 doses vaccine
dPop(a+nVPos2)=dPop(a+nVPos2) - pop(a+nVPos2)*WaningSpeed(2);
dPop(a+wVPos)=dPop(a+wVPos) + pop(a+nVPos2)*WaningSpeed(2) - pop(a+wVPos)*WaningSpeed(4);
dPop(a+WaneVPos)=dPop(a+WaneVPos) + pop(a+wVPos)*WaningSpeed(4);

%and 3 doses vaccine 
dPop(a+nVPos3)=dPop(a+nVPos3) - pop(a+nVPos3)*WSB;
dPop(a+WaneVPos)=dPop(a+WaneVPos) + pop(a+nVPos3)*WSB;
...
dPop(a+nVPos3) = dPop(a+nVPos3) + V3.*(pop(a+nVPos2)+pop(a+WaneVPos)+pop(a+wVPos)+pop(a+WaneRPos))./Frac_UnBoosted;
...
dPop(a+RPos) = dPop(a+RPos) + V3.*pop(a+wRPos)./Frac_UnBoosted;
